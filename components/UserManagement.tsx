...
const UserManagement: React.FC<UserManagementProps> = ({ onClose }) => {
    ...
    const [stationAccess, setStationAccess] = useState('All');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await createNewUser(email, password, role, stationAccess);
            setMessage({ type: 'success', text: t('userCreated') });
            setEmail('');
            setPassword('');
            setRole('user');
            setStationAccess('All');
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 modal-overlay animate-fade-in" onClick={onClose}>
            <div className="bg-white dark:bg-[#191E26] w-full max-w-4xl rounded-xl shadow-2xl overflow-hidden animate-scale-in flex flex-col md:flex-row max-h-[80vh]" onClick={e => e.stopPropagation()}>
                
                {/* Form Section */}
                <div className={`w-full md:w-1/2 p-6 flex flex-col border-b md:border-b-0 ${dir === 'rtl' ? 'md:border-l' : 'md:border-r'} border-[#D5D9D9] dark:border-gray-700 overflow-y-auto`}>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4" dir={dir}>
                        ...
                        <div>
                            <label className="block text-sm font-bold text-[#232F3E] dark:text-gray-300 mb-1">{t('role') || 'الصلاحية'}</label>
                            <select
                                value={role}
                                onChange={(e) => setRole(e.target.value)}
                                className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 focus:ring-2 focus:ring-[#FF9900] focus:border-[#FF9900] outline-none transition-all bg-white dark:bg-gray-700 dark:text-white"
                            >
                                <option value="user">كابتن (Captain)</option>
                                <option value="manager">مشرف فرعي (Manager)</option>
                                <option value="admin">مدير رئيسي (Main Admin)</option>
                            </select>
                        </div>
                        <div>
                            <label className="block text-sm font-bold text-[#232F3E] dark:text-gray-300 mb-1">{t('station') || 'محطة العرض'}</label>
                            <select
                                value={stationAccess}
                                onChange={(e) => setStationAccess(e.target.value)}
                                className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 focus:ring-2 focus:ring-[#FF9900] focus:border-[#FF9900] outline-none transition-all bg-white dark:bg-gray-700 dark:text-white"
                            >
                        ...
                        <div className="pt-4">
                            <button 
                                type="submit" 
                                disabled={loading}
                                className="w-full bg-[#FFD814] hover:bg-[#F7CA00] text-[#0F1111] border border-[#FCD200] font-bold py-2 rounded-lg shadow-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center gap-2"
                            >
                ...
                <div className="w-full md:w-1/2 bg-gray-50 dark:bg-[#111315] p-6 flex flex-col h-full overflow-hidden">
                    ...
                    <div className="flex-1 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                        {isLoadingList ? (
                            ...
                            usersList.map((user, idx) => (
                                <div key={idx} className="bg-white dark:bg-[#191E26] p-3 rounded-lg border border-[#D5D9D9] dark:border-gray-700 shadow-sm flex items-center justify-between group hover:border-[#FF9900] transition-colors">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div className="text-sm font-bold text-[#0F1111] dark:text-white">
                                                ...
                                                {user.email !== ADMIN_EMAIL && user.role === 'manager' && <span className="mx-2 text-[10px] bg-purple-100 text-purple-800 border border-purple-200 px-1 rounded">مشرف فرعي (Manager)</span>}
                                                {user.email !== ADMIN_EMAIL && user.role === 'user' && <span className="mx-2 text-[10px] bg-gray-100 text-gray-800 border border-gray-200 px-1 rounded">كابتن (Captain)</span>}
                                                {user.email !== ADMIN_EMAIL && user.role === 'admin' && <span className="mx-2 text-[10px] bg-red-100 text-red-800 border border-red-200 px-1 rounded">مدير رئيسي (Admin)</span>}
                                                {user.email !== ADMIN_EMAIL && user.stationAccess && user.stationAccess !== 'All' && <span className="mx-2 text-[10px] bg-blue-100 text-blue-800 border border-blue-200 px-1 rounded">{user.stationAccess}</span>}
                    ...
                    <div className="mt-4 text-[10px] text-gray-400 text-center bg-white dark:bg-[#191E26] p-2 rounded border border-[#D5D9D9] dark:border-gray-700">
