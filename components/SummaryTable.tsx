...
const BadgeIcon: React.FC<{ type: string }> = ({ type }) => {
    const config: any = {
        sniper: { icon: 'fa-crosshairs', color: 'text-rose-500', bg: 'bg-rose-100', title: 'Sniper' },
        turbo: { icon: 'fa-jet-fighter', color: 'text-blue-500', bg: 'bg-blue-100', title: 'Turbo' },
        guardian: { icon: 'fa-shield-halved', color: 'text-emerald-500', bg: 'bg-emerald-100', title: 'Guardian' },
        fire: { icon: 'fa-fire', color: 'text-amber-500', bg: 'bg-amber-100', title: 'On Fire' },
        beast: { icon: 'fa-dumbbell', color: 'text-purple-500', bg: 'bg-purple-100', title: 'Beast' }
    };
    const s = config[type] || { icon: 'fa-star', color: 'text-gray-500', bg: 'bg-gray-100' };

    return (
        <span className={`w-5 h-5 rounded-full ${s.bg} ${s.color} flex items-center justify-center text-[10px] shadow-sm animate-pulse-soft`} title={s.title}>
            <i className={`fa-solid ${s.icon}`}></i>
        </span>
    );
};

const EditableCell = ({ value, agentName, field, onUpdate }: { value: number, agentName: string, field: keyof DASummary, onUpdate: any }) => {
...
const EditableNameCell = ({ name, isSuperAdmin, onUpdate }: { name: string, isSuperAdmin: boolean, onUpdate: (old: string, newName: string) => void }) => {
    ...
    return (
        <div className="flex items-center gap-2 group/edit">
            <span 
                ...
                className={`font-black text-[15px] tracking-tight text-gray-900 dark:text-white ${isSuperAdmin ? "cursor-pointer hover:text-[#FF9900] transition-colors" : ""}`}
            >
                {name}
            </span>
            {isSuperAdmin && (
...
const SummaryTable: React.FC<SummaryTableProps> = ({ data, sortConfig, onSort, onUpdateValue, onViewDetails, isSuperAdmin, onUpdateName, searchQuery = '' }) => {
  const { t } = useSettings();
  
  const getSortIcon = (key: keyof DASummary) => {
    if (sortConfig?.key !== key) return <i className="fa-solid fa-sort mr-1 opacity-20 text-[10px]"></i>;
    return sortConfig.direction === 'asc' ? <i className="fa-solid fa-caret-up mr-1 text-[#FF9900]"></i> : <i className="fa-solid fa-caret-down mr-1 text-[#FF9900]"></i>;
  };
          ...
          <table className="w-full text-center border-collapse custom-table">
            <thead>
              <tr className="bg-[#232F3E] text-white">
                <th className="no-print w-10 !bg-[#232F3E] !border-b-[#37475A]"></th>
                <th onClick={() => onSort('daName')} className="!bg-[#232F3E] !text-white w-1/3 text-right pr-6 cursor-pointer hover:bg-[#37475A] transition-colors group !border-b-[#37475A]">
                    {t('agentName')} {getSortIcon('daName')}
                </th>
                <th onClick={() => onSort('delivered')} className="!bg-[#232F3E] !text-white cursor-pointer hover:bg-[#37475A] transition-colors !border-b-[#37475A]">{t('delivered')} {getSortIcon('delivered')}</th>
                <th onClick={() => onSort('failed')} className="!bg-[#232F3E] !text-white cursor-pointer hover:bg-[#37475A] transition-colors !border-b-[#37475A]">{t('failed')} {getSortIcon('failed')}</th>
                <th onClick={() => onSort('ofd')} className="!bg-[#232F3E] !text-white cursor-pointer hover:bg-[#37475A] transition-colors !border-b-[#37475A]">{t('ofd')} {getSortIcon('ofd')}</th>
                <th onClick={() => onSort('rto')} className="!bg-[#232F3E] !text-white cursor-pointer hover:bg-[#37475A] transition-colors !border-b-[#37475A]">{t('rto')} {getSortIcon('rto')}</th>
                <th onClick={() => onSort('total')} className="!bg-[#232F3E] !text-white cursor-pointer hover:bg-[#37475A] transition-colors !border-b-[#37475A]">{t('total')} {getSortIcon('total')}</th>
                <th onClick={() => onSort('successRate')} className="!bg-[#232F3E] !text-white w-1/4 cursor-pointer hover:bg-[#37475A] transition-colors !border-b-[#37475A]">{t('performance')} {getSortIcon('successRate')}</th>
              </tr>
            </thead>
            <tbody className="bg-white dark:bg-[#191E26]">
              {data.summaries.map((s, idx) => {
                const match = searchQuery && searchQuery.length > 2 
                    ? s.allTrackings?.find(t => t.id.toLowerCase().includes(searchQuery.toLowerCase())) 
                    : null;

                return (
                <tr key={idx} className={`group border-b border-gray-100 dark:border-gray-800 hover:bg-[#f2f8fa] dark:hover:bg-[#232F3E] transition-colors ${match ? 'bg-yellow-50/50 dark:bg-yellow-900/10' : ''}`}>
                  <td className="no-print py-4">
                    <button 
                      onClick={() => onViewDetails(s)}
                  ...
                  <td className="text-[#007185] dark:text-[#4DB6AC] font-bold text-lg py-4">
                    <EditableCell value={s.delivered} agentName={s.daName} field="delivered" onUpdate={onUpdateValue} />
                  </td>
                  <td className="text-[#CC0C39] dark:text-[#E57373] font-bold text-lg py-4">
                    <EditableCell value={s.failed} agentName={s.daName} field="failed" onUpdate={onUpdateValue} />
                  </td>
                  <td className="text-[#C45500] dark:text-[#FFB74D] font-bold text-lg py-4">
                    <EditableCell value={s.ofd} agentName={s.daName} field="ofd" onUpdate={onUpdateValue} />
                  </td>
                  <td className="text-[#565959] dark:text-[#B0BEC5] font-bold text-lg py-4">
                    <EditableCell value={s.rto} agentName={s.daName} field="rto" onUpdate={onUpdateValue} />
                  </td>
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      <span className={`font-black w-12 text-right tabular-nums text-lg ${getSuccessColor(s.successRate)}`}>
                        {Math.round(s.successRate)}%
                      </span>
                      <div className="flex-1 h-3 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden shadow-inner" dir="ltr">
                        <div 
            ...
            <tfoot className="bg-[#232F3E] text-white">
              <tr>
                <td colSpan={2} className="p-4 text-right pr-6 font-bold uppercase tracking-wider text-sm">{t('stationTotal')}</td>
                <td className="font-mono font-bold text-lg">{data.grandTotal.delivered}</td>
                <td className="font-mono font-bold text-lg text-rose-300">{data.grandTotal.failed}</td>
                <td className="font-mono font-bold text-lg text-amber-300">{data.grandTotal.ofd}</td>
                <td className="font-mono font-bold text-lg text-gray-300">{data.grandTotal.rto}</td>
      ...
      <div className="md:hidden card-view bg-gray-50 dark:bg-gray-800 p-3 rounded-lg">
          ...
          <div className="space-y-3">
            {data.summaries.map((s, idx) => {
                const match = searchQuery && searchQuery.length > 2 
                    ? s.allTrackings?.find(t => t.id.toLowerCase().includes(searchQuery.toLowerCase())) 
                    : null;
                return <MobileCard key={idx} s={s} onViewDetails={onViewDetails} t={t} matchedTracking={match} />;
            })}
          </div>
          ...
          <div className="mt-6 bg-[#232F3E] text-white rounded-xl p-4 shadow-lg">
              ...
              <div className="bg-white/10 rounded-lg p-3 text-center">
                  <span className="block text-[10px] text-gray-300 uppercase mb-1">{t('successRate')}</span>
